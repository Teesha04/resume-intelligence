...
- `pdfplumber` is tried first, then `pypdf`.
...
pipeline. To screen a real set, drop the files into `./resumes` and re-run —
no code changes required. `output/results.json` is produced from the bundled
sample set.
